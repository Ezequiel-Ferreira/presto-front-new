...
export class MesaService extends BaseApi {
  ...
  criarMesa(mesa: Mesa): Observable<any> {
    return this.http.post<any>(
      this.URL_BASE+'/mesa/create/' + this.authService.loggedUser().id,
      mesa
    );
  }
  ...
  getMesaByTime(): Observable<any> {
    return this.http.get<Mesa[]>(
      this.URL_BASE+`/mesa/mesasbytime/${this.authService.loggedUser().id}`
    )
  }

  timerPedido(): Observable<any> {
    return this.http.put<Mesa[]>(
      this.URL_BASE+`/mesa/diminuirtempo/${this.authService.loggedUser().id}`, true
    )
  }
}
