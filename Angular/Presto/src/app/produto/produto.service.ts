...
export class ProdutoService {
  ...
  constructor(private http: HttpClient, private authService : AuthService) { }
   ...
   produtos(): Observable<Produto[]> {
     return this.http.get<Produto[]>("http://localhost:8080/produto/produtousuario/" + this.authService.loggedUser().id);
   }
   adicionarProdutoNocardapio(produto : Produto): Observable<any>{
    return this.http.put<Cardapio>("http://localhost:8080/cardapio/addproduto/" + this.authService.loggedUser().id , produto);
 }
